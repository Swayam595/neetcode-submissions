class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        return self.__rob_bottom_up_optmized(nums)
    ...
    # TC -> O(N)
    # SC -> O(N)
    def __rob_bottom_up(self, nums: List[int]):
    ...
    def __rob_bottom_up_optmized(self, nums: List[int]) -> int:
        take_first_pos = self.__rob_bottom_up_optmized_helper(nums[1:])
        skip_first_pos = self.__rob_bottom_up_optmized_helper(nums[:-1])
        
        return max(take_first_pos, skip_first_pos)

    def __rob_bottom_up_optmized_helper(self, nums: List[int]) -> int:
        n = len(nums)
        if n == 0:
            return 0

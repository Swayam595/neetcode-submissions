class Solution:
    def findMaxForm(self, strs: List[str], m: int, n: int) -> int: 
        ...
        return self.__bottomUpOptimized(zerosAndOnesCount, m, n)  
    ...
    # TC - O(N * n * m)
    # SC - O(N * n * m) + O(N) -> O(N) for max recursive stack len
    ...
    # TC - O(N * n * m)
    # SC - O(n * m) + O(N) -> O(N) for max recursive stack len
    # Where N represents the number of binary strings, m and m 
    # are the maximum allowable counts of zeros and ones, respectively.
    def  __bottomUpOptimized(self, zerosAndOnesCount: List[dict], m: int, n: int) -> int:
        N = len(zerosAndOnesCount)
        ...
        for i in range(N - 1, -1, -1):
            ...
            for j in range(m + 1):
                for k in range(n + 1):
                    ...
                    take = 0
                    if total_0s <= j and total_1s <= k:
                        take = 1 + dp[j - total_0s][k - total_1s]

class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        return self.__trap_2_pointer(height)
    
    # TC -> O(N)
    # SC -> O(1)
    # N -> len of height
    def __trap_2_pointer(self, height: List[int]) -> int:
        n = len(height)
        ...
        while l < r:
            if left_max < right_max:
                ...
                max_water = max_water + left_max - height[l]
            else:
                r -= 1
                right_max = max(right_max, height[r])
                max_water = max_water + right_max - height[r]

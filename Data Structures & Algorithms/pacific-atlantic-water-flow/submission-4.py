class Solution:
    ...
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        return self.__bfs(heights)
    
    # TC -> O(N * M)
    # SC -> O(N * M)
    # N -> # of rows
    # M -> # of cols
    def __bfs(self, heights: List[List[int]]) -> List[List[int]]:
        ans = []
        pacific, atlantic = self.__generate_ocean_array(2)
        ...
        for r in range(self.__N):
            ...
            atlantic_queue.append([r, self.__M - 1])

        for c in range(self.__M):
            pacific_queue.append([0, c])
            atlantic_queue.append([self.__N - 1, c])

        self.__bfs_helper(pacific_queue, heights, pacific)
        self.__bfs_helper(atlantic_queue, heights, atlantic)

        return self.__generate_ans(pacific, atlantic)

    def __bfs_helper(self, q: deque, heights: List[List[int]], ocean:List[List[int]]) -> None:
        while len(q) > 0:
            x, y = q.popleft()
            ocean[x][y] = True
    ...
    def __generate_ans(self, pacific: List[List[int]], atlantic: List[List[int]]) -> List[List[int]]:
        ans = []
    ...
    def __generate_ocean_array(self, count: int) -> List[List[List[int]]]:
        return [
                    [
                        [False] * (self.__M) for _ in range(self.__N)

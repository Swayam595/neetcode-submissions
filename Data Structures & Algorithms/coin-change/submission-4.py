class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        min_coins_needed = self.__bottomUpOptimized(coins, amount)
    ...
    def __bottomUpOptimized(self, coins: List[int], amount: int) -> int | float('inf'):
        N = len(coins)
        M = amount
        ...
        for i in range(N - 1, -1, -1):
            ...
            for j in range(M + 1):
                if j == 0:
                    ...
                    take = float('inf')
                    if coins[i] <= j:
                        take = 1 + new_dp[j - coins[i]]
                    skip = dp[j]
                    new_dp[j] = min(take, skip)
            dp = new_dp

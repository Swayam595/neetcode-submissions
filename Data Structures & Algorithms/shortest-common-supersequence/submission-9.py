class Solution:
    def shortestCommonSupersequence(self, str1: str, str2: str) -> str:
        ...
        return self.__bottomUpOptimized(str1, str2)
    ...
    def __bottomUpOptimized(self, s1: str, s2: str) -> str:
        N = len(s1)
        M = len(s2)
        ...
        for i in range(N - 1, -1, -1):
            ...
            for j in range(M - 1, -1, -1):
                if s1[i] == s2[j]:
                    new_dp[j] = s1[i] + dp[j + 1]
                else:
                    option1 = s1[i] + dp[j]
                    option2 = s2[j] + new_dp[j + 1]

                    new_dp[j] = option1 if len(option1) < len(option2) else option2
